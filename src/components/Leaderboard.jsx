    ...
    return (
      <div className="flex flex-col h-full">
        <div className="px-4 py-3 flex-shrink-0 bg-gradient-to-r from-slate-700 to-slate-800 border-b border-slate-600">
          <div className="h-5 w-32 bg-slate-600 animate-pulse" />
        </div>
        <div className="flex-1 p-2 space-y-1.5">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-10 bg-slate-800/60 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 flex-shrink-0 flex items-center justify-between bg-gradient-to-r from-slate-700 to-slate-800 border-b border-slate-600">
        ...
        <span className="bg-slate-600/50 px-2.5 py-0.5 text-xs text-gray-200">Top 5</span>
      </div>
        ...
        {leaderboard.slice(0, 5).map((user, index) => {
          ...
          return (
            <div
              ...
              className={`${cfg.card} border px-2.5 py-1.5 flex items-center gap-2 cursor-pointer hover:border-blue-400 transition-colors duration-150`}
            ...
            >
              ...
              <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 ring-1 ring-black/5">
                {user.profile_picture
                  ? <img src={cldOptimize(user.profile_picture, { width: 80, height: 80 })} alt="" className="w-full h-full object-cover" loading="lazy" decoding="async" />
              ...
              {user.project_image && (
                <div className="w-8 h-8 flex-shrink-0 overflow-hidden border border-slate-200">
                  <img src={cldOptimize(user.project_image, { width: 80, height: 80 })} alt="" className="w-full h-full object-cover" loading="lazy" decoding="async" />
                </div>
              )}

              {/* Wins */}
              <div className={`flex-shrink-0 flex items-center gap-0.5 px-2 py-0.5 ${cfg.badgeBg}`}>
                <span className={`text-xs font-bold ${cfg.badgeText}`}>{user.total_wins}</span>
                <span className={`text-[10px] ${cfg.badgeText} opacity-70`}>W</span>
              </div>
            </div>
          );
        })}
